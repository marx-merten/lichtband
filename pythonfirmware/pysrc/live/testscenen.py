...
class BlockLauflicht (LEDScene):
    def __init__(self, steps_per_second=10, finish=160, muster=[], speed=1):
        super().__init__()
    ...
    def tick(self, deltaFrames: int) -> list:
        ...
        stop = start+len(self.start_values)
        for led in range(start, stop):
            ml = len(self.muster)
            self.controller.write_pixel(led, self.muster[(led-start-self.offset) % ml])
