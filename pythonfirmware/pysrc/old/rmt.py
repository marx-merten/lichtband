...
class Pixels:
    ...
    def __setitem__(self, pixel_index, colors):
        ...
        if isinstance(pixel_index, int):
            ...
            self_pixels[pixel_index] = tuple(clocks for bit in (D_ONE if ((channel >> bit) & 1)
                                                                else D_ZERO for channel in colors for bit in range(CHANNEL_WIDTH - 1, -1, -1)) for clocks in bit)
            ...
            if isinstance(colors[0], int):
                ...
                for index in range(start, stop, step):
                    ...
                    self_pixels[index] = tuple(clocks for bit in (D_ONE if ((channel >> bit) & 1)
                                                                  else D_ZERO for channel in colors for bit in range(CHANNEL_WIDTH - 1, -1, -1)) for clocks in bit)
            else:
                # pixels[:] = [(r,g,b), ...]
                ...
                for index in range(start, stop, step):
                    ...
                    self_pixels[index] = tuple(clocks for bit in (D_ONE if ((channel >> bit) & 1)
                                                                  else D_ZERO for channel in color for bit in range(CHANNEL_WIDTH - 1, -1, -1)) for clocks in bit)
        else:
